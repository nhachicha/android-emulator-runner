    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (!fs.existsSync(avdPath) || forceAvdCreation) {
                ...
                yield exec.exec(`sh -c \\"echo no | avdmanager create avd --force -n "${avdName}" --abi '${arch}' --package 'system-images;android-${apiLevel};${target};${arch}' ${profileOption} ${sdcardPathOrSizeOption} -p /home/runner/.android/avd/"`);
            }
            if (cores) {
                yield exec.exec(`sh -c \\"printf 'hw.cpu.ncore=${cores}\n' >> ${process.env.ANDROID_AVD_HOME}/"${avdName}".avd"/config.ini`);
            }
            yield exec.exec(`sh -c \\"ls -l ${process.env.ANDROID_AVD_HOME}/"`);
            yield exec.exec(`sh -c \\"ls -l /home/runner/.android/avd/"`);
            yield exec.exec(`sh -c \\"ls -l ${process.env.ANDROID_AVD_HOME}/"${avdName}".avd/"`);
            yield exec.exec(`sh -c \\"ls -l ${process.env.ANDROID_AVD_HOME}/"${avdName}".avd"/config.ini`);
            if (ramSize) {
